...
x1 <- runif(n,5,15)  #menyebar uniform
x2 <- rnorm(n,3,3)   #menyebar normal
x0 <- rep(1,n)
...
```{r}
par(mfrow=c(1,2))
plot(x1, y, pch=19) #untuk indikasi awal aja
plot(x2, y, pch=19)
```
...
```{r}
sigma_kuadrat <- (t(y)%*%y-t(b)%*%t(X)%*%y)/(n-p) # p nya 3 karna B0 B1 B2
...
## Signifikansi Parameter (nilai t) (uji secara parsial)
