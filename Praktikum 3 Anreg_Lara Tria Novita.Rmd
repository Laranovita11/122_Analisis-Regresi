...
```{r}
library(readxl)
dt <- read_xlsx("C:/Users/Hp/OneDrive/Desktop/Documents/Analisis Regresi/Indeks Pembangunan Literasi Masyarakat dan Unsur Penyusunnya Menurut Provinsi, 2024.xlsx")
dt
```

```{r}
y <- dt$"Indeks Pembangunan Literasi Masyarakat"
x <- dt$"Tingkat Kunjungan Masyarakat per hari"
n <- length(dt)
```
...
Berdasarkan perhitungan, didapat dugaan persamaan regresi linear sebagai berikut. Dugaan persamaan tersebut dapat pula disebut sebagai nilai harapan dari peubah respons Y (Indeks Pembangunan Literasi Masyarakat)

$$E[\hat{Y}] = \hat{Y} = 59.717 + 26.183 X$$
...
$$JKT = JKR + JKG$$
Nilai penguraian keragaman ini dapat diamati pula menggunakan fungsi anova pada bagian **Sum Sq (Jumlah Kuadrat)** seperti yang ditampilkan di bawah. Pada output sintaks di bawah, JKR model bernilai 1471.0 dan JKG model bernilai 2883.4

```{r}
(anova.model <- anova(model))
```

Dari tabel ANOVA dan penguraian keragaman, dapat diduga pula ragam galat/error dari model yang kita peroleh. Secara umum, error dari model dapat diduga oleh nilai Kuadrat Tengah Galat (KTG) sehingga berdasarkan output sintaks di atas, diketahui bahwa ragam galat dari model bernilai 77.93051. Hubungan antara keduanya dapat dituliskan sebagai berikut.
...
Oleh karena simpangan baku merupakan akar kuadrat dari ragam, maka nilai dugaan galat baku model yang kita bentuk adalah $S_e = \sqrt{77.93051} \approx 8.827826$
...
Sebelumnya, melalui fungsi `lm` dan perhitungan manual yang sudah dibahas pada pertemuan sebelumnya, kita sudah mendapatkan nilai dugaan parameter β0 dan β1 sebesar 59.71733 dan 26.1835. Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [51.33103 ,  68.10362] dan untuk parameter $β_1$ adalah [11.93252 ,  40.43448]
...
predict(model, amatan.diduga, interval = "confidence") #kalau rataan confidance kalau individu prediction
```

Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=10 adalah 321.5523. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=10 berada dalam selang 203.4154 hingga 439.6892.
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=10 adalah 321.5523. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=10 berada dalam selang 202.069 hingga 441.0357.